...
def twoStageAdmm_denoise_bayer(y_bayer, Phi_bayer, _lambda=1, gamma=0.01,
                ...
                X_orig=None, model_denoise=None,model_demosaic=None, show_iqa=True,demosaic_method = 'malvar2004',lr_=0.000001,
                 inital_iter=1,interval_iter=5,logf = None,useGPU=True,update_=False,update_per_iter=1,close_form_demosaic=False,large=False):
            ...
            if k<0 or denoiser=='tv': # total variation (TV) denoising
                ...
                if close_form_demosaic and k>0:
                    ...
                    for imask in range(nmask):
                        ...
                        x_rgb = test_ddnet(x_bayer_3ch,model=model_demosaic)
                ...
                if update_ and k>inital_iter and k%interval_iter==0:
                    ...
                    xbgr3,model_denoise = fastdvdnet_denoiser_full_tensor_v2(x_rgb_w,nsig,yall, Phiall,model_denoise,useGPU,lr_,update_,update_per_iter)
                else:
    ...
    return cuda2np(xbgr3), x_bayer_np,psnr_, ssim_, psnr_all,model_denoise # numpy

def gap_denoise_bayer_demosaic(y_bayer, Phi_bayer, _lambda=1, gamma=0.01,
                denoiser='tv', iter_max=50, noise_estimate=True, sigma=None, 
                 x0_bayer=None, 
                X_orig=None, model_denoise=None,model_demosaic=None, show_iqa=True,demosaic_method = 'malvar2004',lr_=0.000001,
                 inital_iter=1,interval_iter=5,logf = None,useGPU=True,update_=False,update_per_iter=1,close_form_demosaic=False,large=False):
    ...
    for idx, nsig in tqdm.tqdm(enumerate(sigma)): # iterate all noise levels
        for it in range(iter_max[idx]): 
            ...
            if k<0 or denoiser=='tv': # total variation (TV) denoising
                ...
                if update_ and k>inital_iter and k%interval_iter==0:
                    xbgr3, model_denoise = ffdnet_rgb_denoise_full_tensor(x_rgb,yall, Phiall, nsig,model_denoise,useGPU,lr_,update_,update_per_iter)
                else:   
                    xbgr3 = ffdnet_rgb_denoise_full_tensor(x_rgb,yall, Phiall, nsig,model_denoise,useGPU,lr_)
                ...
                if update_ and k>inital_iter and k%interval_iter==0:
                    torch.cuda.empty_cache()
                    xbgr3,model_denoise = fastdvdnet_denoiser_full_tensor_v2(x_rgb,nsig,yall, Phiall,model_denoise,useGPU,lr_,update_,update_per_iter)
                else:
                    xbgr3 = fastdvdnet_denoiser_full_tensor_v2(x_rgb,nsig,yall, Phiall,model_denoise,useGPU,lr_)
                # xbgr3 = xbgr3.permute()
                xall[...,0] = xbgr3[0::2,0::2,0,:] # R  channel (average over two)
                xall[...,1] = xbgr3[0::2,1::2,1,:] # G1=G2 channel (average over two)
                xall[...,2] = xbgr3[1::2,0::2,1,:] # G2=G1 channel (average over two)
            ...
            # io.imwrite('7_dn.png',cuda2np(xbgr3[:,:,:,7]))
