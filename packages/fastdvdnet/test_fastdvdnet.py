...
from utils.utils_image import add_gaussian_noise_meas_cuda,fourCh2OneCh,gen_masked_data
...
def fastdvdnet_denoiser_full_tensor_v2(vnoisy, sigma, y_bayer=None,Phi=None,model=None, 
useGPU=True,lr_=0.000001,updata_=False,update_per_iter=1,gray=False,update_times=-1):
	r"""Denoise an input video (H x W x F x C for color video, and H x W x F for
	     grayscale video) with FastDVDnet
	"""
	# start_time = time.time()
	...
	if updata_:
		if type(update_per_iter)==type(1):
			n_update_iter = [update_per_iter]
			lr_all = [lr_]
		else: # type(update_per_iter)==type([1]):
			n_update_iter = update_per_iter
			lr_all = lr_
		# for n_lr in n_update_iter:		
		mse = nn.MSELoss().cuda() # nn.L1Loss().cuda() #SSIMLoss().cuda()#torch
		...
		vnoisy_plus = vnoisy +add_gaussian_noise_meas_cuda(vnoisy,5/255).float()       
		if mask:
			vnoisy_plus = gen_masked_data(vnoisy_plus,'b',ratio=0.1)
		Phi_bayer = fourCh2OneCh(Phi)
		y_bayer_onech = fourCh2OneCh(y_bayer)
		
		if fix:
			model.module.temp2.train()
			for m in model.module.temp2.modules():
				if isinstance(m,nn.BatchNorm2d):
					m.eval()
			# model.module.temp1.train()
			# for m in model.module.temp1.modules():
			# 	if isinstance(m,nn.BatchNorm2d):
			# 		m.eval()
		else:
			# pass
		...
		for iter_id, lr_i in enumerate(lr_all):
			torch.cuda.empty_cache()
			# model.train()
			optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_i)  # using ADAM opt
			# model.eval()
			...
			for _ in range(n_update_iter[iter_id]):	
				...
				for frameidx in range(N):
					...
					noisy_seq = vnoisy_plus[idx].reshape((1, -1, H, W)) # reshape from [W, C, H, W] to [1, W*C, H, W]
				...
				x_bayer = gen_bayer_img(outv,1)

				up_meas = torch.sum(x_bayer * Phi_bayer, dim=2)
				total_loss=mse(up_meas,y_bayer_onech)	
