...
def demosaicing_CFA_Bayer_Malvar2004_tensor(CFA,R_m, G_m, B_m, pattern='RGGB'):
    ...
    # dim1,dim2 = Rg_RB_Bg_BR.shape
    Rg_BR_Bg_RB =Rg_RB_Bg_BR.permute(1,0)

    Rb_BB_Br_RR = as_array(
        [[0, 0, -1.5, 0, 0],
         [0, 2, 0, 2, 0],
         [-1.5, 0, 6, 0, -1.5],
         [0, 2, 0, 2, 0],
    ...
    CFA = CFA.unsqueeze(0).unsqueeze(0)
    GR_GB = GR_GB.unsqueeze(0).unsqueeze(0).float()
    Rg_RB_Bg_BR = Rg_RB_Bg_BR.unsqueeze(0).unsqueeze(0).float()
    Rg_BR_Bg_RB = Rg_BR_Bg_RB.unsqueeze(0).unsqueeze(0).float()
    Rb_BB_Br_RR = Rb_BB_Br_RR.unsqueeze(0).unsqueeze(0).float()
    
    CFA = F.pad(CFA,(2,2,2,2),'reflect')

    conv = F.conv2d(CFA, GR_GB,stride=1).squeeze(0).squeeze(0)
    G = torch.where(torch.logical_or(R_m == 1, B_m == 1), conv, G)

    RBg_RBBR = F.conv2d(CFA, Rg_RB_Bg_BR,stride=1).squeeze(0).squeeze(0)
    RBg_BRRB = F.conv2d(CFA, Rg_BR_Bg_RB,stride=1).squeeze(0).squeeze(0)
    RBgr_BBRR = F.conv2d(CFA, Rb_BB_Br_RR,stride=1).squeeze(0).squeeze(0)
...
#     G = torch.where(torch.logical_or(R_m == 1, B_m == 1), np2tch_cuda(convolve(CFA_np, GR_GB)), G)

#     RBg_RBBR = np2tch_cuda(convolve(CFA_np, Rg_RB_Bg_BR))
#     RBg_BRRB = np2tch_cuda(convolve(CFA_np, Rg_BR_Bg_RB))
#     RBgr_BBRR = np2tch_cuda(convolve(CFA_np, Rb_BB_Br_RR))
