...
def get_patch(sample, patch_size=512, scale=1, multi=False, input_large=False):
    frames,ch, ih, iw = sample.shape
    ...
    # iframe = random.randrange(0, frames-5)
...
def open_sequence_train(seq_dir, gray_mode, expand_if_needed=False, max_num_fr=100):
	...
	for fpath in files:#[start_img_id:start_img_id + max_num_fr]:
...
def open_image(fpath, gray_mode, expand_if_needed=False, expand_axis0=True, normalize_data=True):
	...
	if not gray_mode:
		...
		img = cv2.imread(fpath) #[0:480,0:800,:]
