...
def save_out_seq(seq,seqnoisy, seqclean, save_dir, sigmaval, suffix, save_noisy):
	...
	for idx in range(seq_len):
		...
		noisy_name = os.path.join(save_dir,\
						('n{}_Noisy_{}').format(sigmaval, idx) + fext)
		origin_name = os.path.join(save_dir,('n{}_Origin_{}').format(sigmaval, idx) + fext)
		if len(suffix) == 0:
...
def test_fastdvdnet(**args):
	...
	if not args['dont_save_results']:
		# Save sequence
		save_out_seq(seq,seqn, denframes.clip(0,1), args['save_path'], \
