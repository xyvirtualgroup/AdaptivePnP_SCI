...
from DDnet_test import denoise_seq_ddnet
...
def lr_scheduler(epoch, argdict,current_lr=None):
	...
	reset_orthog = False
	if epoch > argdict['milestone'][0]:
		current_lr = argdict['lr'] / 2.
	elif epoch % argdict['milestone'][1]:
		current_lr = argdict['lr'] / 4
	elif epoch % argdict['milestone'][2]:
		current_lr = argdict['lr'] / 8
	elif epoch % argdict['milestone'][3]:
...
# 		  training_params['step'])
	...
	print("[epoch {}] loss: {:1.4f} time:{}".\
		  format(epoch+1, loss.item(),time_str))
...
def validate_and_log(model_temp, dataset_val, valnoisestd, temp_psz, writer, \
	...
	with torch.no_grad():
		for seq_val in dataset_val:
			...
			for i in range(seqn_val.shape[0]):
				CFA, CFA4, mosaic, mask = mosaic_CFA_Bayer_cuda(seqn_val[i, :, :, :].permute(1, 2, 0))
				seqn_val[i, :, :, :] = mosaic.permute(2, 0, 1)


			out_val = denoise_seq_ddnet(seq=seqn_val, \
