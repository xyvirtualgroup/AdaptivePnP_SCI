...
from models.network_demosaicking import FastDVDnet
from packages.DDnet.DDnet_test import denoise_seq_fastdvdnet
...
def save_out_seq(seq,seqnoisy, seqclean, save_dir, sigmaval, suffix, save_noisy):
	...
	for idx in range(seq_len):
		...
		noisy_name = os.path.join(save_dir,\
						('n{}_{}_Noisy').format(sigmaval, idx) + fext)
		origin_name = os.path.join(save_dir,('n{}_{}_Origin').format(sigmaval, idx) + fext)
		if len(suffix) == 0:
			out_name = os.path.join(save_dir,\
					('n{}_{}_FastDVDnet').format(sigmaval, idx) + fext)
		else:
			out_name = os.path.join(save_dir,\
					('n{}_{}_{}_FastDVDnet').format(sigmaval, suffix, idx) + fext)

		# Save result
		if save_noisy:
			# noisyimg = variable_to_cv2_image(seqnoisy[idx].clamp(0., 1.))
			# cv2.imwrite(noisy_name, noisyimg)
			io.imwrite(noisy_name, seqnoisy[idx].permute(1,2,0).cpu().detach().numpy())


		# outimg = variable_to_cv2_image(seqclean[idx].unsqueeze(dim=0))
		# cv2.imwrite(out_name, outimg)
		io.imwrite(out_name, seqclean[idx].permute(1,2,0).cpu().detach().numpy())
		io.imwrite(origin_name, seq[idx].permute(1,2,0).cpu().detach().numpy())
def test_fastdvdnet(**args):
	...
	with torch.no_grad():
		...
		for i in range(seqn.shape[0]):
			CFA, CFA4, mosaic, mask = mosaic_CFA_Bayer_cuda(seqn[i, :, :, :].permute(1, 2, 0))
			seqn[i,:, :, :] = mosaic.permute(2, 0, 1)
	...
	stop_time = time.time()
	# denframes = denframes.clip(0,1)
	import  numpy as np
	denframes = (denframes - denframes.min()) / (denframes.max() - denframes.min())
	# import skimage
	# skimage.util.dtype
	# import cv2 as cv
	# cv.normalize(denframes, dst=dst, alpha=0, beta=1.0, norm_type=cv.NORM_MINMAX)
	...
	if not args['dont_save_results']:
		# Save sequence
		save_out_seq(seq,seqn, denframes, args['save_path'], \
					   args['noise_sigma']*255, args['suffix'], args['save_noisy'])
...
if __name__ == "__main__":
	# Parse arguments
	# 'D:\PyProjects\fastdvdnet-master\DAVIS-2017-trainval-480p\DAVIS\JPEGImages\480p'
	parser = argparse.ArgumentParser(description="Denoise a sequence with FastDVDnet")
	parser.add_argument("--model_file", type=str,\
						default="joint_logs/ckpt_e28601.pth", \
						help='path to model of the pretrained denoiser')
	parser.add_argument("--test_path", type=str, default="DAVIS-2017-trainval-480p\DAVIS\JPEGImages\\480p\hockey", \
	...
	parser.add_argument("--save_noisy", action='store_true',default=True, help="save noisy frames")
	parser.add_argument("--no_gpu", action='store_true', help="run model on CPU")
	parser.add_argument("--save_path", type=str, default='./joint_results', \
