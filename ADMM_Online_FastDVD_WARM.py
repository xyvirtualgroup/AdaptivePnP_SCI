...
from dvp_linear_inv_ADMM_tensor import (admm_denoise_bayer_demosaic,admm_denoise, np2tch_cuda,  )
# from dvp_linear_inv_2_stage_ADMM import admm_denoise_bayer_demosaic_pre as reconstruct
# from dvp_linear_inv_2_stage_ADMM import admm_denoise_bayer_demosaic as reconstruct
from dvp_linear_inv_2_stage_ADMM_tensor_online import admm_denoise_bayer_demosaic_pre as reconstruct # work
# from dvp_linear_inv_ADMM_tensor import admm_denoise_bayer_demosaic as reconstruct # origin
from utilspy import *
from packages.fastdvdnet.models import FastDVDnet
from models.network_demosaicking import DDnet
# from models.network_demosaicking import FastDVDnet as DDnet
...
for ncount in range(0,6):
    ...
    v_twoStageAdmm_fastdvd_bayer = np.zeros([nrows, ncols, nmask*nmea], dtype=np.float32)
    psnr_fastdvd_bayer = np.zeros([nmask*nmea,1], dtype=np.float32)
    ssim_fastdvd_bayer = np.zeros([nmask*nmea,1], dtype=np.float32)
    ...
    for iframe in range(nmea):
        ...
        # v_tv,_,_,_ =   \
        #     twoStageADMM(meas_bayer_t, mask_bayer, _lambda,
        #                                   0.01, 'tv', 20, noise_estimate, 0,x0_bayer=None,
        #                                   X_orig=orig_bayer_t, 
        ...
        onlineFastDVDnet[iframe,:,:,:,:],v_twoStageAdmm_FastDVDnet_bayer_t,\
            psnr_twoStageAdmm_FastDVDnet_bayer_t,ssim_twoStageAdmm_FastDVDnet_bayer_t,psnrall_FastDVDnet_bayer_t,refined_model =   \
        ...
        end_time = time.time()
        t_twoStageAdmm_FastDVDnet_bayer = end_time - begin_time
        print('ADMM-{}--{}-{} PSNR {:2.2f} dB, SSIM {:.4f}, running time {:.1f} seconds.'.format(
            denoiser.upper(),datname,iframe ,mean(psnr_twoStageAdmm_FastDVDnet_bayer_t), mean(ssim_twoStageAdmm_FastDVDnet_bayer_t), t_twoStageAdmm_FastDVDnet_bayer))
        f.write('ADMM-{}--{}-{} PSNR {:2.2f} dB, SSIM {:.4f}, running time {:.1f} seconds. \n'.format(
            denoiser.upper(),datname,iframe , mean(psnr_twoStageAdmm_FastDVDnet_bayer_t), mean(ssim_twoStageAdmm_FastDVDnet_bayer_t), t_twoStageAdmm_FastDVDnet_bayer))
        v_twoStageAdmm_fastdvd_bayer[:,:,iframe*nmask:(iframe+1)*nmask] =  v_twoStageAdmm_FastDVDnet_bayer_t
        psnr_fastdvd_bayer[iframe*nmask:(iframe+1)*nmask,:] = np.reshape(psnr_twoStageAdmm_FastDVDnet_bayer_t,(nmask,1))
        ssim_fastdvd_bayer[iframe*nmask:(iframe+1)*nmask,:] = np.reshape(ssim_twoStageAdmm_FastDVDnet_bayer_t,(nmask,1))
        # In[12]:

        

        average_psnr_i.append(mean(psnr_twoStageAdmm_FastDVDnet_bayer_t))  # mean of 8 frames
        average_ssim_i.append(mean(ssim_twoStageAdmm_FastDVDnet_bayer_t))
        psnrall_FastDVDnet_bayer.append(psnrall_FastDVDnet_bayer_t)
...
# print(sigma*255)
    ...
    sio.savemat('{}Admm_{}_{}{:d}_sigma{:d}_all7_log.mat'.format(savedmatdir,denoiser.lower(),datname,nmask,int(sigma[-1]*MAXB)),
                {

                    'v_twoStageAdmm_fastdvd_bayer':v_twoStageAdmm_fastdvd_bayer,
                    'psnr_fastdvd_bayer':psnr_fastdvd_bayer,
                    'ssim_fastdvd_bayer':ssim_fastdvd_bayer,
                    'onlineFastDVDnet':onlineFastDVDnet,
                    'psnr_all_iter':psnrall_FastDVDnet_bayer,
