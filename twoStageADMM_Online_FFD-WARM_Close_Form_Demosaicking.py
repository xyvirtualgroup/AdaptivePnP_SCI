...
from dvp_linear_inv_2_stage_ADMM_tensor_online import twoStageAdmm_denoise_bayer as reconstruct
...
for ncount in range(0,6):
    ...
    if ncount == 0:
        ...
        sigma = [15/255]  #    pre-set noise standard deviation
        iter_max = [8]  # maximum number of iterations
        ## pre params
    ...
    warm_start_file = './results/savedmat/_twoStageAdmm_fastdvd_color_'+datname+'8_sigma6_all7_log.mat'

    file = sio.loadmat(warm_start_file) # for '-v7.3' .mat file (MATLAB)
        # print(list(file.keys()))
    recon_tv = np.array(file['v_twoStageAdmm_fastdvd_gray_bayer'])
    ...
    for iframe in range(nmea):
        ...
        onlineffdnet[iframe,:,:,:,:],v_twoStageAdmm_ffdnet_gray_bayer_t,\
            ...
            reconstruct(meas_bayer_t, mask_bayer, _lambda,
                                            ...
                                            close_form_demosaic=close_form_demosaic)  
...
# print(sigma*255)
    ...
    sio.savemat('{}_twoStageAdmm_{}_{}{:d}_sigma{:d}_all7_log.mat'.format(savedmatdir,denoiser.lower(),datname,nmask,int(sigma[-1]*MAXB)),
                {
